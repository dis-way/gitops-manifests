...
| `apps` | Standard variant; enables Traefik CRD provider, adds HSTS middleware and root catch-all `IngressRoute` |
| `multitenancy` | Gateway API variant; Flux resources in `platform-system`, `kubernetesCRD` disabled, four Gateway listeners (http/https + wildcard), Linkerd policies included |
| `adminservices` | Gateway API variant; stays in `traefik` namespace, no Linkerd policies |
