...
| `default-gateway` | Data plane: the `eg` EnvoyProxy, the `eg` GatewayClass and Gateway, and the External Secrets wiring that syncs the `tls-cert` Secret from Azure Key Vault. Depends on the namespace from `base` |
